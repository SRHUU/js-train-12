...
function getKeysStartingWith(dictionary, letter) {
  ...
  // Повертаємо масив
  const arr = [];
  for (const existKey of dictionary.keys()) {
    if (existKey[0] === letter) {
      arr.push(existKey);
    }
  }
  return arr;
...
function sortByValues(dictionary) {
  ...
  const arrResult = [...dictionary.entries()];
  arrResult.sort((elem1, elem2) => elem2[1] - elem1[1]);
  const map = new Map(arrResult);
  return map;
}
